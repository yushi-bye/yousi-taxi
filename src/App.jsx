...
export default function App(){
  ...
  const [mode, setMode] = useState('passenger') // passenger, driver, admin
  ...
  return (
    <div style={{height:'100vh', display:'flex', flexDirection:'column'}}>
      <header style={{background:'#111827', color:'white', padding:'12px 20px', display:'flex', justifyContent:'space-between', alignItems:'center'}}>
        <b style={{color:'#FACC15'}}>Yousi 玉里叫車</b>
        <div style={{display:'flex', gap:8}}>
          ...
          <button onClick={()=>{localStorage.removeItem('yousi_user'); setUser(null)}} style={{marginLeft:12}}>登出</button>
        </div>
      </header>
